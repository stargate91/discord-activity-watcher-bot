...
class ElitesCog(commands.Cog):
    ...
    async def _setup_roles(self, guild):
        ...
        if "roles" not in config_data: config_data["roles"] = {}
        if "elite_roles" not in config_data: config_data["elite_roles"] = {}

        for key, data in role_configs.items():
            if key in ["title", "footer"]: continue
            name = data.get("name", key)
            color_str = data.get("color", "0xFFFFFF")
            try: color_int = int(str(color_str), 16)
            except: color_int = 0xFFFFFF
            ...
            if role:
                if role.name != name or role.color.value != color_int:
                    ...
                    except discord.Forbidden: log.warning(f"Forbidden: Could not update role {name}.")
            else:
    ...
    async def _run_elite_logic(self, guild, force=False):
        ...
        if not force:
            if now.weekday() != 0: return # Monday is 0
            last_run = await self.db.get_last_elites_run_date(guild.id)
        ...
        stats = await self.db.get_weekly_elite_stats(guild.id, start_date, end_date)
        ...
        last_elites = await self.db.get_last_elites(guild.id)
        roles_to_remove = set()
        ...
        for category, user_id in last_elites.items():
            ...
            if member:
                ...
                if role_id: roles_to_remove.add((member, role_id))
        ...
        for cat_id, (data, role_id, msg_template) in categories.items():
            ...
            await self.db.log_elite_win(uid, guild.id, cat_id, today)
            ...
            wins = await self.db.get_elite_wins(uid, guild.id)
        ...
        if stats_channel:
            ...
            log.info(f"Weekly Elites announced in {stats_channel.name}")
    ...
    async def force_calculate_elites(self, interaction: discord.Interaction):
        ...
        try:
            await self._run_elite_logic(interaction.guild, force=True)
    ...
    def _get_eligible_elite(self, guild, candidates):
        ...
        for row in candidates:
            ...
            if exclude_role_id != 0:
                if discord.utils.get(member.roles, id=exclude_role_id): continue
            return (uid, val)
        return None
    ...
    async def elite_log(self, interaction: discord.Interaction):
        ...
        async with self.db.pool.acquire() as conn:
            rows = await conn.fetch("""
                ...
                WHERE guild_id = $1 GROUP BY user_id, category ORDER BY wins DESC
        ...
        user_wins = {}
        for r in rows:
            uid, cat, count = r['user_id'], r['category'], r['wins']
    ...
    async def test_weekly_layout(self, interaction: discord.Interaction):
        try:
            ...
            dummy_hof = [Messages.ELITE_HALL_OF_FAME.format(name=f"**{interaction.user.display_name}**", threshold=Config.ELITE_WIN_THRESHOLD)]
    ...
    async def weekly_chances(self, interaction: discord.Interaction):
        ...
        stats = await self.db.get_weekly_elite_stats(interaction.guild_id, start_date, today)
        caller_stats = await self.db.get_user_weekly_elite_stats(interaction.user.id, interaction.guild_id, start_date, today)
        ...
        try:
            view = ModernElitesView(interaction.guild, elite_data, title=Messages.WEEKLY_STANDINGS_TITLE, footer=Messages.WEEKLY_STANDINGS_FOOTER, caller_id=interaction.user.id, caller_stats=caller_stats)
