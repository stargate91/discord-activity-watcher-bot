...
class GameTracker:
    ...
    async def load_franchises(self):
        ...
        for guild in self.bot.guilds:
            db_games = await self.db.get_tracked_games(guild.id)
            
            if not db_games:
                # Fallback to defaults from config + save them to DB for first run
                defaults = Config.DEFAULT_GAMES
                for sub, suf in defaults.items():
                    await self.db.add_tracked_game(guild.id, sub, suf)
                self.franchises[guild.id] = defaults
            else:
                self.franchises[guild.id] = db_games
        
        # Load active sessions from DB
        db_sessions = await self.db.get_active_game_sessions()
        self.active_sessions = db_sessions
        log.info(f"Loaded tracked games for {len(self.franchises)} guilds and {len(self.active_sessions)} active game sessions.")

    def _get_games(self, member):
        """This helper function finds all the games someone is currently playing on their Discord status!"""
        games = set()
        if not member.guild: return games
        
        guild_franchises = self.franchises.get(member.guild.id, {})
        
        for activity in member.activities:
            if activity.type == discord.ActivityType.playing and activity.name:
                name = activity.name
                # Standardize if matched
                for franchise_key, role_suffix in guild_franchises.items():
        ...
        for game in current_games:
            ...
            if self.playing_counts[key] == 1:
                ...
                if key not in self.active_sessions:
                    ...
                    await self.db.start_game_session(main_id, member.guild.id, game, now)
                    ...
                    if role:
                        ...
                        if role not in member.roles:
                            try:
                                ...
                                await self.db.log_role(main_id, member.guild.id, game, action='ADDED')
                            except discord.Forbidden:
                                pass
                
                # Update records about this game
                await self.db.update_game_activity(main_id, member.guild.id, game, bot_assigned=bot_assigned)
    ...
    async def handle_presence_update(self, before, after):
        ...
        if after_spotify and not before_spotify:
            ...
            if key not in self.active_sessions:
                now = datetime.datetime.now(datetime.timezone.utc)
                self.active_sessions[key] = now
                await self.db.start_game_session(main_id, after.guild.id, "Spotify", now)
        
        # Spotify Stopped
        if before_spotify and not after_spotify:
            key = (main_id, after.guild.id, "Spotify")
            start_time = await self.db.end_game_session(main_id, after.guild.id, "Spotify")
            ...
            if start_time:
                ...
                if duration > 0.1:
                    await self.db.add_spotify_minutes(main_id, after.guild.id, duration)
        ...
        for game in (after_games - before_games):
            ...
            if self.playing_counts[key] == 1:
                ...
                if key not in self.active_sessions:
                    now = datetime.datetime.now(datetime.timezone.utc)
                    self.active_sessions[key] = now
                    await self.db.start_game_session(main_id, after.guild.id, game, now)
                        ...
                        for m in linked_members:
                            if role not in m.roles:
                                try:
                                    ...
                                    await self.db.log_role(main_id, after.guild.id, game, action='ADDED')
                                except discord.Forbidden: pass

                await self.db.update_game_activity(main_id, after.guild.id, game, bot_assigned=bot_assigned)
        ...
        for game in (before_games - after_games):
            ...
            if self.playing_counts[key] == 0:
                ...
                start_time = await self.db.end_game_session(main_id, after.guild.id, game)
                if key in self.active_sessions:
                    start_time = self.active_sessions.pop(key)
                ...
                if start_time:
                    ...
                    duration = (datetime.datetime.now(datetime.timezone.utc) - start_time).total_seconds() / 60
                    if duration > 0.1:
                        await self.db.add_game_minutes(main_id, after.guild.id, game, duration)
    ...
    async def cleanup_inactive_roles(self, bot):
        ...
        for guild in bot.guilds:
            inactive = await self.db.get_inactive_games(guild.id, days=Config.INACTIVE_GAME_DAYS)
            for uid, role_name in inactive:
                ...
                if role and role in member.roles:
                    try:
                        ...
                        await self.db.remove_game_activity(uid, guild.id, role_name)
                        await self.db.log_role(uid, guild.id, role_name, action='REMOVED')
                    except discord.Forbidden: pass
